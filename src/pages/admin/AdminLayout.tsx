...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col md:flex-row">
      <aside className="border-b md:border-b-0 md:border-r border-border bg-surface md:w-56 shrink-0">
        <div className="flex items-center justify-between p-4 md:block">
          <div>
            <p className="font-semibold text-sm">Admin</p>
            <p className="text-xs text-muted truncate">{user?.email}</p>
          </div>
          <Link to="/dashboard" className="md:mt-3 hidden md:flex items-center gap-1 text-xs text-accent-light hover:underline">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm whitespace-nowrap transition-colors ${
                  active ? 'bg-accent/20 text-accent-light' : 'text-muted hover:bg-surface-2 hover:text-text'
                }`}
              >
